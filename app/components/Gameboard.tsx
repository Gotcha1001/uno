...
// 1. Import useSoundManager
// 2. Play card colour sound on handleCardClick (after validation)
// 3. Play cardDraw sound on handleDraw
// 4. Play yourTurn chime when isMyTurn flips true (useEffect)
// 5. Play unoAlert when playerHand.length === 1 (useEffect)
// 6. Play win/lose when game.status === "finished" (useEffect)
// 7. Play cardDeal once when the game first becomes active (useEffect)
// 8. MuteButton component rendered in header
// ─────────────────────────────────────────────────────────────────────────────
...
import { Id } from "@/convex/_generated/dataModel";
import { useBackground } from "../context/BackgroundContext";
import { useSoundManager } from "@/hooks/useSoundManager"; // ← adjust path as needed
...
export function GameBoard({
  ...
  // Track previous turn & hand length for reactive sounds
  ...
  // 1. Deal sound once when game becomes active
  useEffect(() => {
  ...
  // 2. "Your turn" chime whenever the turn flips to us
  useEffect(() => {
  ...
  // 3. UNO alert when hand drops to 1 card
  useEffect(() => {
  ...
  // 4. Win / lose sound when game finishes
  useEffect(() => {
  ...
  const handleCardClick = async (cardId: string) => {
    ...
    // ── Play colour-tinted sound immediately on click ──
    if (value === "wild" || cardId === "wild_draw4") {
      play("cardPlayWild");
    } else {
      play("cardPlay", color);
